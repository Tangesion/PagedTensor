...
namespace paged_tensor::common
{
    ...
    public:
        ...
        DataPtr(std::vector<void *> *blockMap, size_t offsetStart, size_t blockSize, size_t typeSize)
            : blockMap(blockMap), offsetStart(offsetStart), blockSize(blockSize), typeSize(typeSize), cachedBlock(nullptr), cachedBlockIndex(-1)
        {
        }
        DataPtr() : offsetStart(0), blockMap(nullptr), cachedBlock(nullptr), cachedBlockIndex(-1) {}
    ...
    public:
        ...
        T &data(size_t index)
        {
            size_t offset = index + offsetStart;
            ...
            float *blockPtr = static_cast<T *>(cachedBlock);
            return blockPtr[blockOffset];
        }
        // T *data()
        //{
