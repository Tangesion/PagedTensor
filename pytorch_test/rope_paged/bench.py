...
lib_path = os.path.join(os.path.dirname(__file__), '../build/rope_paged/rope_paged.cpython-310-x86_64-linux-gnu.so')
if not os.path.exists(lib_path):
    raise ImportError(f"Cannot find shared library: {lib_path}")
sys.path.append(os.path.join(current_dir, '../build/rope_paged'))

from rope_paged import precomputeFreqsCosSin
from rope_paged import applyRope
...
max_len = 4096
theta = 10000
result = precomputeFreqsCosSin(dim, max_len, theta)
#print(result.shape)
#print(result)
#print(result.shape)

#q = torch.randn(1, 1, 32, 128, dtype=torch.float32)
q = torch.randn(1, 1024, 32, 128, dtype=torch.float32)
q_clone = q.clone()
position = torch.arange(0, 1024).to(torch.uint64)
result_rope = applyRope(q, result, position)
print(result_rope)
#print(result.shape)
...
position = torch.arange(0, 1024).unsqueeze(0)
